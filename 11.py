...
class CallSession:
    def __init__(self, name: str, phone: str, call_sid: str, session_id: str):
        ...
        # VAD State for smart interruption
...
async def text_to_speech_stream(session: CallSession, text: str, ws: WebSocket, stream_sid: str):
    """Streams TTS audio to Twilio for clearer voice quality."""
    session.is_speaking = True
    
    try:
        # Stream audio from Deepgram for better quality
...
async def media_stream(ws: WebSocket):
    ...
    async def on_deepgram_message(data: dict):
        ...
        # === SMART VAD LOGIC START ===
        if msg_type == "SpeechStarted":
            # Don't stop yet! Wait for words to confirm it's not noise.
            session.waiting_for_speech_confirmation = True
            # Fail-safe: If no transcript comes in 500ms, reset flag.
            ...
            # Check if we got words after a SpeechStarted event
            if session.waiting_for_speech_confirmation:
                ...
                # If there is text, it's a REAL interruption. Stop the bot.
                if transcript:
            ...
            if is_final and transcript:
                ...
                # Double check we aren't speaking
                if session.is_speaking:
                    await session.stop_speaking()
...
async def call_lead(lead: dict):
    try:
        # FIX: Create session BEFORE making the call with UUID
